...
module.exports = {
        ...
        .addSubcommand((sub) =>
          sub
            ...
            .addIntegerOption((opt) =>
              opt.setName("amount").setRequired(true).setDescription("XP value")
            )
        )
        .addSubcommand((sub) =>
          sub
  ...
  async execute(interaction) {
    ...
    if (sub === "addxp") {
      ...
      return interaction.reply({
        content:
          `✅ Added \`${amount}\` XP to ${user.tag}. New XP: \`${stats.xp}\`, Level: \`${stats.level}\`` +
          (result.leveledUp
    ...
    if (sub === "removerole") {
      ...
      if (member && role && member.roles.cache.has(role.id)) {
        ...
        return interaction.reply({
          content: `✅ Removed role **${roleName}** from ${user.tag}.`,
          ephemeral: true,
        });
      } else {
        return interaction.reply({
          content: `⚠️ User doesn't have role ${roleName}.`,
          ephemeral: true,
        });
      }
    }

    ...
    async execute(interaction) {
      ...
      if (!durationMs || durationMs < 1000 || durationMs > 28 * 24 * 60 * 60 * 1000) {
        return interaction.reply({
          content: '❌ Invalid duration. Use formats like `10m`, `2h`, or `1d` (max 28d).',
          flags: 1 << 6
        });
      }
      ...
      if (!member) {
        return interaction.reply({ content: '❌ User not found in this server.', flags: 1 << 6 });
      }
  
      if (!member.moderatable) {
        return interaction.reply({ content: `⚠️ I can’t mute ${target.tag}.`, flags: 1 << 6 });
      }
      ...
      try {
        ...
        securityLog.log(`🔇 Timed mute: ${target.tag} for ${rawDuration} by ${interaction.user.tag} — ${reason}`);
        ...
        return interaction.reply({ content: '❌ Failed to apply timeout.', flags: 1 << 6 });
      }
    }
  };

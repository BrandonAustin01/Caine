  ...
  async execute(interaction) {
    if (isRateLimited(interaction.user.id, 5000)) {
      return interaction.reply({
        content: "⏳ Slow down! Try again shortly.",
        ephemeral: true,
      });
    }
    ...
    if (mode === "on") {
      ...
      return interaction.reply(
        "🔒 Lockdown mode is now **ENABLED**. All new joins will be blocked."
      );
    }
    ...
    if (mode === "off") {
      ...
      return interaction.reply(
        "🔓 Lockdown mode is now **DISABLED**. New members can join normally."
      );
    }
  },
};

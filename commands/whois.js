...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const isMuted =
      member?.roles.cache.some((r) => r.name.toLowerCase().includes("muted")) ||
      false;
    ...
    const perms =
      member?.permissions?.toArray().slice(0, 5).join(", ") || "N/A";
    ...
    securityLog.log(
      `📋 Whois lookup on ${user.tag} (${user.id}) by ${interaction.user.tag}`
    );

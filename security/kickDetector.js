...
module.exports = function detectKickAbuse(guildId, kickedUser, executorTag) {
  ...
  if (valid.length >= config.maxKicks) {
    sendAlert(require('discord.js').client, `⚠️ Cain: ${valid.length} kicks detected in 10s in ${guildId}. Auto-lockdown triggered.`);
    const alert = `⚠️ ALERT: ${valid.length} kicks in ${config.intervalMs / 1000}s in guild ${guildId}. Last executor: ${executorTag}`;
    logger.error(alert);

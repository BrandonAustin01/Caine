...
module.exports = async function detectRaid(client, member) {
  ...
  logger.info(`👥 ${validJoins.length} join(s) in ${member.guild.name} within ${config.intervalMs / 1000}s`);

  if (validJoins.length >= config.maxJoins) {
    const alert = `🚨 **Cain Alert**: Raid detected in **${member.guild.name}**.\n• Joins: ${validJoins.length} in ${config.intervalMs / 1000}s\n• Punishment: ${config.punishment.toUpperCase()}`;
    sendAlert(alert);

    logger.warn(`🚨 Possible raid detected in ${member.guild.name} — taking action`);
    securityLog.log(`🚨 Raid triggered in ${member.guild.name} — ${validJoins.length} joins`);
    ...
    try {
      ...
      const newMembers = guildMembers.filter(m =>
        now - m.joinedTimestamp < config.intervalMs
      );
      ...
      for (const [id, m] of newMembers) {
        ...
        if (config.punishment === 'kick') {
          ...
          securityLog.log(`👢 Raid kick: ${m.user.tag} in ${member.guild.name}`);
